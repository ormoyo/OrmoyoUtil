...
public class AbilityEventListenerImpl<T extends Event> implements AbilityEventListener<T>
{
    private static final AtomicInteger IDs = new AtomicInteger();
    private static final String HANDLER_DESC = Type.getInternalName(AbilityEventListener.class);
    ...
    private final AbilityEventListener<T> handler;
    ...
    @Override
    public void invoke(Ability ability, T event)
    {
    ...
    public Class<T> createWrapper(Method callback)
    {
        if (cache.containsKey(callback))
        {
            return (Class<T>) cache.get(callback);
        }
